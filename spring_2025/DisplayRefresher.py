...
def update_display():
    ...
    while not correct_size:
        ...
        if correct_size:
            while not ((hours_bbox[3] > HEIGHT - MARGIN) or (
                    hours_bbox[2] > (WIDTH - MARGIN - 250))):  # while the hours bbox is not too big
                small_font_size += 1
                small_font = small_font = ImageFont.truetype(font=os.path.join(picdir, 'Font.ttc'),
